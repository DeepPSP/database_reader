...
def preprocess_img(img:np.ndarray, rescale:Real=1/255, resize:ArrayLike=(299,299), dtype:type=np.float32):
    """
    """
...
def grad_cam_keras(input_model:Model, img:np.ndarray, target_layer:Union[str, int, Layer], prediction_layer:Optional[Union[str, int, Layer]]=None, **kwargs) -> Tuple[np.ndarray, np.ndarray, np.ndarray, int, str]:
    ...
    if verbose >= 1:
        print("prediction_layer is {}, the {}-th layer of `inpput_model`".format(pl_name, pl_idx))
        print("target_layer is {}, the {}-th layer of `inpput_model`".format(tl_name, tl_idx))
    ...
    # grads = _compute_gradients(pred_output, [target_output])[0]
    # grads = _compute_gradients(loss, [target_output])[0]
    # print('grads =', grads)
    ...
    if verbose >= 2:
        ...
        _, (ax1,ax2,ax3) = plt.subplots(1,3,figsize=(10,30))
        ax1.imshow(img)
        ax2.imshow(cam)
...
def grad_cam_plusplus_keras(input_model:Model, img:np.ndarray, target_layer:Union[str, int, Layer], prediction_layer:Optional[Union[str, int, Layer]]=None, **kwargs) -> Tuple[np.ndarray, np.ndarray, np.ndarray, int, str]:
    """
    Grad-CAM++
