...
class SleepAccel(OtherDataBase):
    ...
    def plot_mt_lb(self, subject_id:str, bin_state:bool=False):
        """
        """
        fig,ax_lb = plt.subplots(figsize=(20,4))
        lb_fp = os.path.join(self.lb_path, subject_id+self.lb_file_suffix)
        df_lb = pd.read_csv(lb_fp,sep=' ',header=None,names=['sec','sleep_stage'])
        if bin_state:
    ...
    def plot_ct_lb(self, subject_id:str, bin_state:bool=False):
        """
        """
        fig,ax_lb = plt.subplots(figsize=(20,4))
        lb_fp = os.path.join(self.lb_path, subject_id+self.lb_file_suffix)
        ...
        x_rsmpl = resample_irregular_timeseries(acc_data[:,[0,1]], output_fs=50, return_with_time=True, method='interp1d', options={})
        y_rsmpl = resample_irregular_timeseries(acc_data[:,[0,2]], output_fs=50, return_with_time=True,method='interp1d', options={})
        z_rsmpl = resample_irregular_timeseries(acc_data[:,[0,3]], output_fs=50, return_with_time=True,method='interp1d', options={})
    ...
    def acc_to_count(self, df_acc:pd.DataFrame, freq:Real, epoch_len:Real) -> np.ndarray:
        """
        """
